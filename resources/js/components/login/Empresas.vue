        ...
        <v-row>
          <v-col v-for="empresa in empresas" :key="empresa.id" xs="12">
            ...
            >
              ...
              ></v-img>
              <v-img contain v-else height="250" :src="empresa.logo"></v-img>
          ...
          <div>
            <div v-if="empresas == null">
              ...
              <v-img contain height="500" src="img/lapizRoto.png"></v-img>
            </div>
          </div>
        </v-row>
      </v-container>
  ...
  methods: {
    allEmpresas() {
      axios.defaults.headers.common["Authorization"] =
        "Bearer " + localStorage.getItem("token"); // obtener el token de la aplicaccion
      axios
        .get("/api/allEmpresas/" + this.$store.state.user.id)
        .then((response) => {

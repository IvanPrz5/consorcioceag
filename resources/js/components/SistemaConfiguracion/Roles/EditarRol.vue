...
<v-form ref="form" enctype="multipart/form-data">
    ...
    <v-container>
        <v-row>
            ...
            <v-col cols="12">
                <v-text-field v-model="rol.descripcion" :rules="letrasYnumeros" placeholder="Ingrese la descripcion del rol" label="Descripcion" outlined clearable prepend-inner-icon="mdi-map-marker" counter required></v-text-field>
            </v-col>
            ...
            <v-col cols="12" md="6" sm="4">
                <v-switch v-model="rol.estatus" inset clearable></v-switch>
            </v-col>
    ...
    methods: {
        Nuevo() {
            if (this.$refs.form.validate()) {
                ...
                axios.post('/api/roles/editar', this.rol)
                    .then(response => {
                        console.log(response);
                        this.mostrarNoificacion("Nuevo Rol", false);
                        this.barraActiva = false;
                        this.cerrarDialogo.value = false;
                        this.$refs.form.resetValidation();
                        this.$emit("Guardado")
                    }).catch(error => {
                        this.mostrarNoificacion("No es posible guardar el rol", true);
                        this.barraActiva = false;
                    });
            } else {
